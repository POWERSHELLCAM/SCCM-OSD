'Variable declaration. Fill the blank field and pass the resource id and collection id as parameter
Option Explicit
...
If scriptmode = "ADD" Then
	If Failcode = 0 then
		'add new system to New Systems collection
		Log ("--------------------------------------------------------")
		returnVal = ""
		returnVal = AddMachineToCollection(resourceID, collectionID, smsSiteNamespace )
		If returnVal = False Then
			Failcode = -2
		End If
	End If
...
Function AddMachineToCollection(resourceID, collectionID, smsSiteNamespace )
